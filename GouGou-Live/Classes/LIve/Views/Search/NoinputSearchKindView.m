...
#import "NoinputSearchKindView.h"
#import "SearchLiveKindBtnView.h"//按钮

@interface NoinputSearchKindView ()<UITableViewDelegate, UITableViewDataSource>
...
static NSString *cellid = @"SearchLiveKindBtnView";

@implementation NoinputSearchKindView
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 1) {
        NSArray *historyArr = [[NSUserDefaults standardUserDefaults] arrayForKey:History];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        hotView.clickBlcok = ^(NSString *kind){
            if (_typeBlock) {
                _typeBlock(kind);
            }
        };
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:self.historyDataArr forKey:History];
...
- (void)deleAllHistorydata {
    ...
    NSArray * myArray = [userDefaults arrayForKey:History];
    NSMutableArray *searTXT = [myArray mutableCopy];
    [searTXT removeAllObjects];
    [userDefaults setObject:searTXT forKey:History];
    self.historyArr = [userDefaults arrayForKey:History];
